...
- [x] ~~Definir `dashboardPassword` en cada tenant que se quiera activar~~. La contraseña compartida se retiró: para activarle el dashboard a un cliente ahora se le crea al menos un Tenant User (Tenant Users → nuevo, con su Tenant y su rol). Un tenant sin usuarios ve el mensaje de "dashboard no disponible" en vez del login.
